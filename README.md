...
- **智能跳过无权限论文**：自动检测并跳过无访问权限的论文（区分订阅问题和 rate limit）
- 下载失败自动重试（可配置重试次数和间隔）
- **智能速率限制**：
  - 可配置每小时下载配额（默认 100/小时）
  - 随机化请求间隔（避免固定模式被检测）
  - 自适应延迟（根据成功率自动调整）
  - 指数退避 + 随机抖动（遇到 rate limit 时）
...
- **PDF 缓存**：缓存文件列表避免重复扫描
- **下载队列**：支持添加单篇论文到下载队列，可调整优先级顺序
- **分页显示**：论文库支持分页浏览，每页 20 篇，提升大量论文时的性能
...
- **Settings**：设置页面，配置主题、重试策略、每小时下载配额等
...
- 使用筛选器按状态过滤论文（支持筛选队列中的论文）
- 分页浏览：每页显示 20 篇论文，支持翻页导航
- 批量操作：重试失败、删除、导出

### 下载队列

- 点击论文卡片上的 **+** 按钮将单篇论文添加到队列
- 点击右上角队列徽章查看队列内容
- 在队列对话框中可以：
  - 调整下载优先级（上移/下移）
  - 移除单篇论文
